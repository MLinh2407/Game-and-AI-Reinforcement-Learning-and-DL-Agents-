"""
Evaluation script for directional control scheme.
...
def evaluate_model(model_path="models/ppo_directional", num_episodes=5):
    """
    ...
    if not os.path.exists(model_file):
        ...
        print(f"Please train the model first using train_directional.py")
        sys.exit(1)
    
    print("=" * 60)
    print("Evaluating PPO Agent - Directional Control Scheme")
    print("=" * 60)
    ...
    env = ArenaEnvironment(control_scheme=config.CONTROL_DIRECTIONAL, render_mode='human')
...
if __name__ == "__main__":
    ...
    model_path = sys.argv[1] if len(sys.argv) > 1 else "models/ppo_directional"
