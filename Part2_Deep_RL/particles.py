...
class ParticleSystem:
    ...
    def thrust_particle(self, pos, vel):
        ...
        particle = Particle(
            pos[0], pos[1], vx, vy,
            config.COLOR_THRUST,
            config.PARTICLE_THRUST_LIFE
        )
        self.particles.append(particle)
    
    # Create explosion for destroyed enemy
    def enemy_explosion(self, pos):
        self.create_explosion(pos, config.COLOR_EXPLOSION_ENEMY, 
                            config.PARTICLE_EXPLOSION_COUNT_MEDIUM)
    
    # Create explosion for destroyed spawner
    def spawner_explosion(self, pos):
        self.create_explosion(pos, config.COLOR_EXPLOSION_SPAWNER, 
                            config.PARTICLE_EXPLOSION_COUNT_LARGE)
    
    # Create explosion for player death
    def player_explosion(self, pos):
        self.create_explosion(pos, config.COLOR_EXPLOSION_PLAYER, 
                            config.PARTICLE_EXPLOSION_COUNT_PLAYER)
    
    # Create effect for completing a phase
    def phase_complete_effect(self, pos):
        self.create_explosion(pos, config.COLOR_EXPLOSION_PHASE, 
                            config.PARTICLE_EXPLOSION_COUNT_PHASE)
    
    # Create effect when enemy spawns
    def spawn_effect(self, pos):
        self.create_explosion(pos, config.COLOR_SPAWN, 
